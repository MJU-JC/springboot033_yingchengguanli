<template>
  <div class="main-content">
		    ...
		    label="已选座位[用,号隔开]">
		     <template slot-scope="scope">
                ...
                <el-button v-if="isAuth('dianyingxinxi','查看') && contents.tableBtnIcon == 0" type="success" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">{{ contents.tableBtnFont == 1?'详情':'' }}</el-button>
                                                                <el-button v-if="isAuth('dianyingxinxi','统计') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="goupiaotongjiCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'统计':'' }}</el-button>
                <el-button v-if="isAuth('dianyingxinxi','统计') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="goupiaotongjiCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'统计':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('dianyingxinxi','统计') && contents.tableBtnIcon == 0" type="success" size="mini" @click="goupiaotongjiCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'统计':'' }}</el-button>
    ...
    <add-or-update v-if="addOrUpdateFlag" :parent="this" ref="addOrUpdate"></add-or-update>
            ...
            <goupiaotongji-cross-add-or-update v-if="goupiaotongjiCrossAddOrUpdateFlag" :parent="this" ref="goupiaotongjiCrossaddOrUpdate"></goupiaotongji-cross-add-or-update>
...
import goupiaotongjiCrossAddOrUpdate from "../goupiaotongji/add-or-update";
export default {
  data() {
    return {
                                                                                                                                                                                                                                                                                                                                                searchForm: {
      ...
      addOrUpdateFlag:false,
                  goupiaotongjiCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    contentPageStyleChange(){
      ...
      this.contents.pageEachNum = 10
            ...
            goupiaotongjiCrossAddOrUpdateHandler(row,type){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.goupiaotongjiCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','dianyingxinxi');
      this.$nextTick(() => {
        this.$refs.goupiaotongjiCrossaddOrUpdate.init(row.id,type);
      });
    },
                    init () {
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          },
